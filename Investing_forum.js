...
forumPost.addEventListener('submit', (e) => {
  ...
  if (user != null) {
    db.collection('investing').add({
      blog: forumPost.blogPost.value,
      username: user.displayName,
      resList: []
    })
    .then(() => {
        window.location.href = 'Investing_forum.html';
    });
  } else {
...
function renderPost(doc) {
  ...
  for (let i = 0; i < doc.data().resList.length; i++) {
    db.collection('investingRes').doc(doc.data().resList[i]).get().then(docRes => {
  ...
  postRes.addEventListener('click', (e) => {
    ...
    if (user != null) {
      ...
      .then((docRef) => {
        var docid = docRef.id;
        var postsDoc = db.collection('investing').doc(id);
        postsDoc.update({
...
// display posts
db.collection('investing').get().then(snapshot => {
